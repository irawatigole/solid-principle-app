...
export const dataReducer = (state: State, action: ActionTypes): State => {
    switch (action.type) {
        case Actions.SEARCH:
            const searchData = [...state.searchData].filter(item =>  
              item.name.toLowerCase().indexOf(action.key.toLowerCase()) >= 0 || item.region.toLowerCase().indexOf(action.key.toLowerCase()) >= 0)
